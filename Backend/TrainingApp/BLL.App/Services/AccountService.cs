...
{
    public class AccountService : BaseEntityService<IAccountRepository, IAppUnitOfWork, DAL.App.DTO.Identity.AppUser, BLL.App.DTO.Identity.AppUser>,
        IAccountService
    {
        ...
        public AccountService(IConfiguration configuration, UserManager<AppUser> userManager,
            SignInManager<AppUser> signInManager, IAppUnitOfWork unitOfWork, RoleManager<AppRole> roleManager, IPlayerPositionService playerPositionService)
            : base(unitOfWork, new BaseBLLMapper<DAL.App.DTO.Identity.AppUser, BLL.App.DTO.Identity.AppUser>(), unitOfWork.AccountRepository)
        {
        ...
        {
            var appUserToRemove = await ServiceRepository.FindAsync(dto.Id);
        ...
        private Domain.Identity.AppUser CreateAppUserFromModel(RegisterDTO model)
        {
        ...
        private async Task<UserDTO> MapToAppUserToUserDto(RegisterDTO model, Domain.Identity.AppUser user, string jwt)
        {
            var positions = await playerPositionService.GetPlayerPositions(user.Id);
        ...
        private async Task AddAppRoleToUser(RegisterDTO model, Domain.Identity.AppUser user)
        {
