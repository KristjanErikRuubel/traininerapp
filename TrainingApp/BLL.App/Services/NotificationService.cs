...
{
    public class NotificationService : BaseEntityService<INotificationRepository, IAppUnitOfWork,
            DAL.App.DTO.Notification, BLL.App.DTO.Notification>,
        INotificationService
    {
        private IEmailSender _sender { get; }

        public NotificationService(IAppUnitOfWork unitOfWork, IEmailSender sender)
            : base(unitOfWork, new BaseBLLMapper<DAL.App.DTO.Notification, BLL.App.DTO.Notification>(),
        ...
        {
            _sender.SendEmailAsync(user.email, notification.Title, notification.Content);
        }
